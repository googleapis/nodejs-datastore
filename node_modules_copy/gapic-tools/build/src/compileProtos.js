...
exports.gaxProtos = path.join(require.resolve('google-gax'), '..', '..', 'protos');
...
function fixJsFile(js) {
    ...
    js = js.replace('import * as $protobuf from "protobufjs/minimal"', 'import {protobufMinimal  as $protobuf} from "google-gax/build/src/protobuf.js"');
 ...
 */
async function buildListOfProtos(protoJsonFiles, esm) {
    const result = [];
    for (const file of protoJsonFiles) {
        const directory = path.dirname(file);
        const content = await readFile(file);
        const list = JSON.parse(content.toString()).map((filePath) => 
 ...
 */
async function compileProtos(rootName, protos, options) {
    const extraArgs = [];
    if (options.keepCase) {
...
async function generateRootName(directories) {
    ...
    for (const directory of directories) {
        for (const p of (0, walk_up_path_1.walkUp)(path.resolve(directory, '..'))) {
