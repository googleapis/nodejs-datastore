      ...
      describe('create, retrieve and delete', () => {
        ...
        describe('multi-db support for read and write operations', () => {
          it('should run a query with another database', async () => {
            ...
            const firstDatastore = new Datastore({
              namespace: `${Date.now()}`,
            });
            const postKey = firstDatastore.key(['Post', 'post1']);
            await firstDatastore.save({key: postKey, data: post});
            const query = firstDatastore
              .createQuery('Post')
              .hasAncestor(postKey);
            const [defaultDatastoreResults] = await firstDatastore.runQuery(
              query
            );
            assert.strictEqual(defaultDatastoreResults.length, 1);
            const [entity] = await firstDatastore.get(postKey);
            ...
            // Cleanup
            await firstDatastore.delete(postKey);
          });
